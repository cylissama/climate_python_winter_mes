...
load(filethresh);
fileannual=strcat(dircli,strcat(site,"_DATAinput_monthly"));
load(fileannual);

ny=numel(DATAmonth.year);
nm=numel(DATAmonth.month);
nv=numel(DATAmonth.var);
nd=365;

DATAall=DATAmonth.data;
DATAyear=DATAmonth.year;
%Year 5 data(TAIR;VAR 1) for October is DATAall{10,1}{5,1}{:,1}
...
plot(FDm,'r','LineWidth',2)
hold on
plot(Tau(1)+Tau(2)*xaxis,'r--','LineWidth',2)
grid on
return
